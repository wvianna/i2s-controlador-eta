...
<html>
    ...
    <body> 
        ...
        <table>
            ...
            <tr><td Width=300>Tipo de Opera&ccedil;&atilde;o:</td><td Align=Middle width=100 id='statusLocalRemoto'>Carregando</td></tr>
    ...
    <script>
        ...
        var k = setInterval(function() {loadData7('statusAutoManual',updateData7)},2001);
        ...
        var l = setInterval(function() {loadData8('statusLocalRemoto',updateData8)},2001);
        function loadData8(url, callback){
            var lhttp = new XMLHttpRequest();
            lhttp.onreadystatechange = function(){
                if(this.readyState == 4 && this.status == 200){
                    callback.apply(lhttp);
                }
            };
        ...
        function updateData8(){
            document.getElementById('statusLocalRemoto').innerHTML = this.responseText;
        }
